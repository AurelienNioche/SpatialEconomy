...
# ----------------------------------------------------||| CHOICE ||| ----------------------------------------------- #
    ...
    def choose_update_decision(self, idx):
        ...
        try:
            self.decision[idx] = \
                self.decision_array[int(self.finding_a_partner[idx][-1][1]),  # Last element of his list of tuples (choice, success)
                                    self.decision[idx],
                                    self.choice[idx]]
        except:
            self.decision[idx] = 0
    ...
    def update_estimations_success_averages(self):
        ...
        for idx in range(self.n):
            ...
            for i in range(6):
                ...
                b_list_for_i = finding_a_partner[:, 0] == i
                if np.sum(b_list_for_i) > 0:
                    self.success_averages[idx, i] = np.mean(finding_a_partner[b_list_for_i][:, 1])
                else:
